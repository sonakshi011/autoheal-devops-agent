...
class FailurePayload(BaseModel):
    ...
    step_name: Optional[str] = Field(None, description="Name of the specific step that failed, if identifiable")
    exit_code: int = Field(..., description="Exit code of the failing process")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Time the failure was captured")
    ...
    runner_os: Optional[str] = Field(None, description="Operating system of the runner (e.g., Linux, Windows)")
    python_version: Optional[str] = Field(None, description="Python version used in the environment")
    
    # Classification
    severity: FailureSeverity = Field(default=FailureSeverity.HIGH, description="Estimated severity of the failure")
    failure_type: FailureType = Field(default=FailureType.UNKNOWN, description="Category of the failure based on heuristics")
