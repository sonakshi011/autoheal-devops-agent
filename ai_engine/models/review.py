...
class PRReviewComment(BaseModel):
    ...
    severity: ReviewSeverity = Field(default=ReviewSeverity.INFO, description="The severity level of the finding.")

class PRReviewResponse(BaseModel):
    """Schema for a complete AI-generated PR review."""
    summary: str = Field(..., description="A high-level summary of the PR review.")
    comments: List[PRReviewComment] = Field(default_factory=list, description="List of inline comments.")
    overall_status: str = Field(..., description="A status indicator (e.g., 'approve', 'request_changes', 'comment').")
