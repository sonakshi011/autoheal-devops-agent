...
class ReportsService:
    ...
    def read_and_validate_report(file_path: str) -> dict:
        ...
        if not os.path.exists(file_path):
            raise HTTPException(
                status_code=404, detail=f"Report file {os.path.basename(file_path)} not found."
            )
        ...
        if os.path.getsize(file_path) > MAX_FILE_SIZE_BYTES:
            raise HTTPException(
                status_code=400,
                detail=f"Report file {os.path.basename(file_path)} exceeds 5MB size limit.",
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Report file {os.path.basename(file_path)} contains malformed JSON.",
            )
