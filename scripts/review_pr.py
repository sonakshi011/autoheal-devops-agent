...
from ai_engine.models.review import PRReviewResponse, PRReviewComment
...
def get_pr_diff(gh_token: str, repo_name: str, pr_number: int) -> str:
    ...
    # We use the raw diff format
    response = pr.get_files()
    diff_data = ""
    # Alternative: use requests to get the .diff directly if pr.get_files() is too granular
    # but PyGithub's get_pull(n).get_files() provides file objects.
    # For a full unified diff, we can use the PR's diff_url
    import requests
...
def main():
    ...
    if not all([gh_token, repo_name, pr_number_str]):
        logger.error("Missing required environment variables: GITHUB_TOKEN, GITHUB_REPOSITORY, PR_NUMBER")
        sys.exit(1)
    ...
    for template_name in templates:
        ...
        try:
            ...
            raw_response = ai_client.generate_content_with_retry(
                prompt=prompt,
                response_schema=PRReviewResponse.model_json_schema()
            )
            
            review_data = PRReviewResponse.model_validate_json(raw_response)
            final_summaries.append(f"### {template_name.replace('_', ' ').replace('.j2', '').title()}\n{review_data.summary}")
    ...
    with open("ai_review_response.json", "w", encoding="utf-8") as f:
        json.dump({
            ...
            "comments": [c.model_dump() for c in final_comments]
        }, f, indent=2)

    logger.info(
        f"Stage: Pipeline complete. Artifacts written: ai_review_response.json"
    )
